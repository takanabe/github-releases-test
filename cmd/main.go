...
func main() {
	...
	if !validTag.Match([]byte(tag)) {
		log.Fatal("Version tag must follow the format {major}.{minor}.{patch} or {major}.{minor}.{patch}-{rc#}")
	}
	...
	if !validTarget.Match([]byte(target)) {
		log.Fatal("Target must follow the format release-#")
	}
	...
	if !validPrerelease.Match([]byte(preRelease)) {
		log.Fatal("Prerelease must be true or false")
	}
